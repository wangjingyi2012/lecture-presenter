...
- **Fix:**
  ...
  - **桥不转发点击为翻页**：幻灯片自己处理点击驱动的动画（很多 PPTE 页面靠点击推进 GSAP 动画步骤），点击翻页会导致"点哪都跳下一页"。翻页保持键盘为主。第一版曾加过 `slide-bridge-ready`/`slide-bridge-config` 点击翻页握手，上线后立刻踩中此问题，已回退。
...
- **Regression tests:** `lib.rs` 新增 `slide_bridge_tests`（HTML 注入、非 HTML 跳过、`.HTM` 大小写）。注意 vm 跨上下文对象不能用 `assert.deepEqual`（原型不同），用 JSON 序列化比较。
