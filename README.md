...
**排版规范**：9 条硬性约束，覆盖文案口吻、标点风格、重点高亮、卡片样式、术语弹窗、全局样式管理等维度。规则集内置于 LectureAI 云服务，每次生成自动套用、逐页校验。

**教学法（课程设计层）**：覆盖知识点铺排顺序、概念与数据的配比、实操环节的讲解结构、实战案例的选型标准等。方法论内置于 LectureAI 云服务，规划整套课件时自动遵循。

**设计规则（视觉与交互层）**：覆盖字号分级、布局体系、投影安全区、配色、卡片样式、图片交互、文案口吻等。规则集内置于 LectureAI 云服务，配合自动校验闭环把关。
...
Harness 内置于 LectureAI 云服务，随产品持续迭代，所有用户始终获得最新规则。每次 LectureAI 对话自动注入，保证产出始终合规。这不是简单的提示词工程，而是将**教学方法论 + 视觉设计规范 + 交互能力 + Agent 工具链**编译为结构化的云端知识引擎，在 AI 生成前进行约束、在生成后进行校验，形成闭环质量管控。
...
| 质量 | 高（专家级合规课件） | 取决于所选模型（无 Harness 约束） |
