...
**排版规范**：9 条硬性约束，覆盖文案口吻、标点风格、重点高亮、卡片样式、术语弹窗、全局样式管理等维度。完整规则集仅沉淀在服务器端，不对外公开。

**教学法（课程设计层）**：覆盖知识点铺排顺序、概念与数据的配比、实操环节的讲解结构、实战案例的选型标准等。完整方法论仅沉淀在服务器端，不对外公开。

**设计规则（视觉与交互层）**：覆盖字号分级、布局体系、投影安全区、配色、卡片样式、图片交互、文案口吻等。完整规则集仅沉淀在服务器端，不对外公开。
